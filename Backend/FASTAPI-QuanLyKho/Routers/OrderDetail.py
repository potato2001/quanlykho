...
import model
from model import OrderDetailModel,ProductModel,CustomerModel
...
#Tạo đơn hàng
@router.post("/create_order_detail", summary="Create Order Detail")
async def create_order_detail(
    order_detail: OrderDetailSchema,
    db: Session = Depends(get_database_session),
):
    # Generate a single UUID for the order detail
    order_detail_code = str(uuid.uuid4())

    # Create a new OrderDetailModel instance for each product detail in the list
    new_order_details = []
    for product_detail in order_detail.Products:
        new_order_detail = OrderDetailModel(
            OrderDetailCode=order_detail_code, 
            CustomerID=order_detail.CustomerID,
            ProductID=product_detail.ProductID,
            OrderQuantity=product_detail.OrderQuantity,
            OrderDate=datetime.now().strftime("%Y-%m-%d %H:%M"), 
            Status=order_detail.Status
        )
        new_order_details.append(new_order_detail)

    # Add all new order details to the database
    db.add_all(new_order_details)
    db.commit()

    return new_order_details

@router.post("/receive_order/{order_detail_code}", summary="Update Order Detail Status")
async def receive_order(order_detail_code: str, db: Session = Depends(get_database_session)):
    order_details = db.query(OrderDetailModel).filter(OrderDetailModel.OrderDetailCode == order_detail_code).all()
    
    if not order_details:
        raise HTTPException(status_code=404, detail="Order Details not found")

    # Assuming the status field is named 'Status' in OrderDetailModel
    current_datetime = datetime.now().strftime("%Y-%m-%d %H:%M")
    for order_detail in order_details:
        order_detail.ReceivedDate = current_datetime
        order_detail.Status = 1

    db.commit()

    return {"message": "Order received successfully"}
# @router.get("/order_detail/{order_detail_code}", summary="Get Order Details by OrderDetailCode")
# async def get_order_details(order_detail_code: str, db: Session = Depends(get_database_session)):
#     order_details = db.query(OrderDetailModel).filter(OrderDetailModel.OrderDetailCode == order_detail_code).all()
...
async def get_order_details(order_detail_code: str, db: Session = Depends(get_database_session)):
    ...
    products = []
    customer_name = db.query(CustomerModel).filter(CustomerModel.CustomerID == order_details[0].CustomerID).first().CustomerName
    status = order_details[0].Status
    for order_detail in order_details:
        product = db.query(ProductModel).filter(ProductModel.ProductID == order_detail.ProductID).first()
        products.append({
            "ProductID": order_detail.ProductID,
            "ProductName": product.ProductName,  
            "OrderQuantity": order_detail.OrderQuantity
        })
    return {"CustomerName": customer_name, "Products": products, "Status": status}
